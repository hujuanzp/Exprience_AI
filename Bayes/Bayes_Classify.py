...
def spamTest():
    ...
    读取excel里面的文件并进行标记
    '''
    data = xlrd.open_workbook('text.xlsx')  # 打开xls文件
    table = data.sheets()[0]  # 打开第一张表
    nrows = table.nrows  # 获取表的行数
    for i in range(nrows):  # 循环逐行打印
        if i == 0:  # 跳过第一行
            continue
        alist = table.row_values(i)[0].strip()
        alist = re.sub(r"[0-9\s+\.\!\/_,$%^*()?;；:-【】+\"\']+|[+——！，;:。？、~@#￥%……&*（）]+", " ", alist)  # 去标点符号
        seg_list = jieba.cut(alist)
        wordList = (' '.join(seg_list)).split(' ')
        docList.append(wordList)
        classList.append(0)

    data = xlrd.open_workbook('chat.xlsx')  # 打开xls文件
    table = data.sheets()[0]  # 打开第一张表
    nrows = table.nrows  # 获取表的行数
    for i in range(nrows):  # 循环逐行打印
        if i == 0:  # 跳过第一行
    ...
    trainingSet = list(range(96))
    testSet = []  # 创建存储训练集的索引值的列表和测试集的索引值的列表


    for i in range(10):  # 从数据集中挑选10作为测试数据
